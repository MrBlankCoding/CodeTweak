...
if (window.GMBridge === undefined) {
  function ensureEventListenerProtection(scriptId) {
    ...
    const protectedEventTypes = ['DOMContentLoaded', 'load', 'readystatechange'];
    ...
    function createListenerTracker(isWindow) {
      ...
      return function addEventListener(type, listener, options) {
        if (protectedEventTypes.includes(type) && typeof listener === 'function') {
          ...
          const key = `${isWindow ? 'window' : 'document'}_${type}_${scriptId}`;
          ...
          if (
          ...
          } else if (type === 'load' && isWindow && document.readyState === 'complete') {
            ...
            setTimeout(() => listener.call(document, new Event('readystatechange')), 0);
          }
          ...
          userScriptListeners.forEach((listeners, key) => {
            ...
            listeners.forEach(({ listener, originalOptions }) => {
              ...
              if (target === 'document') {
                ...
                originalMethods.windowAddEventListener.call(window, eventType, listener, options);
              }
            });
          });
  ...
  GMBridge.executeUserScriptWithDependencies = executeUserScriptWithDependencies;
