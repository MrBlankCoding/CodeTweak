...
class AISettings {
  ...
  async loadSettings() {
    try {
      const { aiDomEditorConfigs = [], availableModels = [] } = await chrome.storage.local.get([
        'aiDomEditorConfigs',
        'availableModels',
      ]);
  ...
  renderProfilesList() {
    ...
    if (list.children.length === 0) {
      ...
      empty.textContent = this.t('aiSettingsNoProfiles', 'No profiles yet. Click New Profile.');
  ...
  createDraftProfileItem() {
    ...
    subtitle.textContent = this.t('aiSettingsProfileHintNew', 'New unsaved profile');
  ...
  startNewProfile() {
    ...
    this.elements.activeProfileText.textContent = this.t(
  ...
  updateModelOptions(selectedModel = '') {
    ...
    defaultOption.value = '';
    defaultOption.textContent = this.t('aiSettingsSelectModel', 'Select model...');
  ...
  async resetSettings() {
    if (!confirm(this.t('aiSettingsConfirmReset', 'Reset all AI profiles and model cache?')))
      return;
  ...
  async testConnection() {
    ...
    if (!config.apiKey || !config.endpoint || !config.model) {
      this.showToast(
        this.t('aiSettingsToastNeedConnectionFields', 'Add key, endpoint, and model first'),
        'error'
      );
      return;
    }
    ...
    this.setButtonState(
      ...
      true,
      this.t('aiSettingsTesting', 'Testing...')
    );
    ...
    try {
      ...
      if (response.ok) {
        this.showToast(
          this.t('aiSettingsToastConnectionSuccess', 'Connection successful'),
          'success'
        );
      } else {
        ...
        this.showToast(
          `${this.t('aiSettingsToastConnectionFailed', 'Connection failed')}: ${errorMsg}`,
          'error'
        );
      }
    } catch (error) {
      this.showToast(
        `${this.t('aiSettingsToastConnectionError', 'Connection error')}: ${error.message}`,
        'error'
      );
    } finally {
  ...
  async fetchModelsFromAPI() {
    ...
    if (!provider || !apiKey) {
      this.showToast(
        this.t('aiSettingsToastNeedProviderKey', 'Select provider and enter key first'),
        'error'
      );
      return;
    }
    ...
    if (provider === 'custom') {
      this.showToast(
        this.t(
          'aiSettingsToastCustomSyncUnsupported',
          'Custom endpoint model sync is not supported'
        ),
        'error'
      );
      return;
    ...
    this.setButtonState(
      this.elements.fetchModelsBtn,
      true,
      this.t('aiSettingsSyncing', 'Syncing...')
    );
    ...
    try {
      ...
      this.updateModelOptions();
      this.showToast(
        `${this.t('aiSettingsToastSyncedPrefix', 'Synced')} ${models.length} ${this.t('aiSettingsToastSyncedSuffix', 'models')}`,
        'success'
      );
    } catch (error) {
      this.showToast(
        `${this.t('aiSettingsToastSyncFailed', 'Model sync failed')}: ${error.message}`,
        'error'
      );
    } finally {
      this.setButtonState(
  ...
  setButtonState(button, disabled, loadingText) {
    ...
    if (button.id === 'testConnection') {
      ...
      button.replaceChildren(
        ...
        document.createTextNode(this.t('aiSettingsSyncModels', 'Sync Models'))
      );
    }
