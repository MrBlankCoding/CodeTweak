...
export class EventHandler {
  ...
  setupEventListeners() {
    ...
    document.querySelectorAll('.example-prompt').forEach((prompt) => {
      prompt.addEventListener('click', () => {
        elements.userInput.value = prompt.textContent.replace(/^[^a-zA-Z]+/, '').trim();
        this.editor.uiManager.autoResize(elements.userInput);
        elements.sendBtn.disabled = !elements.userInput.value.trim();
  ...
  setupMessageListener() {
    chrome.runtime.onMessage.addListener((message) => {
      ...
      if (message.action === 'aiElementSelected' || message.action === 'elementSelected') {
  ...
  handleAIResponse(response) {
    ...
    this.editor.chatManager.addMessage('assistant', 'Unexpected response format from AI.', {
      error: true,
    });
  }
  ...
  async _collectDOMSummary(tabId) {
    ...
    if (!response?.success && !response?.summary) {
      throw new Error('Could not access page context. Refresh the page and try again.');
    }
  ...
  async _resolveScriptReference(message) {
    ...
    const promptWithoutRef = message.substring(0, atIndex) + suffix.substring(matched.name.length);
