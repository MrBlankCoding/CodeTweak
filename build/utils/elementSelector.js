(() => {
  ...
  var ElementSelector = class {
    ...
    get actionTemplates() {
      return {
        hide: (selector) => `document.querySelector(${JSON.stringify(selector)})?.style.setProperty('display', 'none', 'important');`,
  ...
  observer.observe(target, { attributes: true, childList: true, characterData: true });
    ...
    getUniqueSelector(el) {
      ...
      if (classes.length) {
        ...
        try {
          if (document.querySelectorAll(comboSel).length === 1)
            return comboSel;
        } catch {
        }
      }
    ...
    createBreadcrumb(node) {
      ...
      const label = node.id ? `${tag}#${node.id}` : node.className ? `${tag}.${String(node.className).split(/\s+/).filter(Boolean)[0] || ""}` : tag;
    ...
    createMenuHeader() {
      ...
      const selector = this.getUniqueSelector(this.state.currentElement);
      const displayText = selector ? selector.length > 50 ? selector.slice(0, 50) + "\u2026" : selector : "No element selected";
    ...
    createMenuBody() {
      ...
      const menuItems = [
        { icon: "\u{1F4CB}", text: "Copy Selector", action: "copy" },
        { icon: "\u{1F6AB}", text: "Hide Element", action: "hide" },
        { icon: "\u274C", text: "Remove Element", action: "remove" },
        { icon: "\u{1F446}", text: "Click Element", action: "click" },
        { icon: "\u{1F440}", text: "Watch Changes", action: "observe" }
      ];
    ...
    createMenuFooter() {
      ...
      footer.innerHTML = "<span>Click item to execute \xB7 Esc: cancel</span>";
      return footer;
    }
    ...
    // Event handlers
    handleMouseMove(e) {
      if (!this.state.isSelecting || this.state.menuOpen)
        return;
