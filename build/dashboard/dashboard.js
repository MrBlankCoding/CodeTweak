(() => {
  ...
  // src/utils/gmApiDefinitions.js
  var GM_API_CATEGORIES = {
    STORAGE: "Storage & Data",
    BROWSER_UI: "Browser & UI",
    RESOURCES_NETWORK: "Resources & Network",
    ADVANCED: "Advanced"
  };
  var GM_API_DEFINITIONS = {
    // Storage & Data APIs
    GM_setValue: {
  ...
  var gmApiFlagToGrant = buildApiToGrantMapping();
  ...
  async function loadScripts(elements, state) {
    try {
      ...
      showNotification2("Error loading scripts", "error");
    }
  }
  ...
  async function toggleScript2(scriptId, enabled) {
    try {
      ...
      if (scriptIndex !== -1) {
        ...
        await chrome.storage.local.set({ scripts });
        chrome.runtime.sendMessage({ action: "scriptsUpdated" });
        showNotification2(`Script ${enabled ? "enabled" : "disabled"}`, "success");
      }
    } catch (error) {
      console.error("Error toggling script:", error);
      showNotification2("Error toggling script", "error");
    }
  }
  ...
  async function deleteScript2(scriptId) {
    ...
    try {
      ...
      showNotification2("Error deleting script", "error");
    }
  }
  async function loadSettings(settingsElements) {
    try {
      ...
      showNotification2("Error loading settings", "error");
    }
  }
  async function saveSettings(settingsElements) {
    try {
      ...
      await chrome.storage.local.set({ settings });
      showNotification2("Settings saved successfully", "success");
      chrome.runtime.sendMessage({ action: "settingsUpdated" });
      applyTheme(settings.darkMode);
    } catch (error) {
      console.error("Error saving settings:", error);
      showNotification2("Failed to save settings: " + error.message, "error");
    }
  }
  async function checkForUpdates2(script) {
    if (!script.updateInfo?.updateUrl) {
      showNotification2("No update URL available", "error");
      return;
    }
    try {
      ...
      if (newVersion === script.version) {
        showNotification2("Script is up to date", "success");
        return;
      }
...
Would you like to update now?`
      ...
      if (updateConfirmed) {
        ...
        if (scriptIndex !== -1) {
          ...
          await chrome.storage.local.set({ scripts });
          chrome.runtime.sendMessage({ action: "scriptsUpdated" });
          showNotification2("Script updated successfully", "success");
          await refreshDashboard();
        }
      }
    } catch (error) {
      console.error("Error checking for updates:", error);
      showNotification2("Error checking for updates: " + error.message, "error");
    }
  }
...
${code}`;
      ...
      showNotification2("Script exported", "success");
    } catch (error) {
      console.error("Export failed:", error);
      showNotification2("Export failed", "error");
    }
  }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    initDashboard();
    import_feather_icons2.default.replace();
  });
})();
