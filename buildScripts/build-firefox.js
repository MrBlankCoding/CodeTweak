...
if (firefoxManifest.content_scripts) {
  firefoxManifest.content_scripts = firefoxManifest.content_scripts.map((cs) => {
  ...
  try {
    ...
    if (/new\s+Function\s*\(\s*['"]unsafeWindow['"]\s*,\s*userCode\s*\)/.test(content)) {
      content = content.replace(
        /new\s+Function\s*\(\s*['"]unsafeWindow['"]\s*,\s*userCode\s*\)/g,
        "(function(){ throw new EvalError('Function constructor is blocked in Firefox'); })"
      );
      changed = true;
    }

    const evalPatterns = [/eval\("this"\)/g, /\(1,\s*eval\)\("this"\)/g];
